...
TEST_CASE("Check size", "[malloc3]") {
  ...
  REQUIRE(b != nullptr);
  after = sbrk(0);
  REQUIRE(aligned_size(11) + _size_meta_data() * 2 ==
          (size_t)after - (size_t)base);
...
TEST_CASE("free", "[malloc3]") {
  ...
  sfree(new_a);
  verify_blocks(3, 10 * 5 + _size_meta_data() * 2, 1,
                10 * 3 + _size_meta_data() * 2);
  verify_size(base);
  sfree(new_b);
  verify_blocks(2, 10 * 5 + _size_meta_data() * 3, 1,
                10 * 4 + _size_meta_data() * 3);
  verify_size(base);
  sfree(new_c);
...
TEST_CASE("free holes 1", "[malloc3]") {
  ...
  verify_size(base);
  sfree(b);
  verify_blocks(4, 10 * 5 + _size_meta_data() * 1, 1,
                10 * 2 + _size_meta_data());
  verify_size(base);
  sfree(new_c);
  verify_blocks(3, 10 * 5 + _size_meta_data() * 2, 1,
                10 * 3 + _size_meta_data() * 2);
