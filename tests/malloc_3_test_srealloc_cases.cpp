...
static inline size_t aligned_size(size_t size) { return size; }

#define verify_blocks(allocated_blocks, allocated_bytes, free_blocks,          \
                      free_bytes)                                              \
  do {                                                                         \
    ...
    REQUIRE(_num_meta_data_bytes() ==                                          \
            aligned_size(_size_meta_data() * allocated_blocks));               \
  } while (0)

#define verify_size(base)                                                      \
  do {                                                                         \
    void *after = sbrk(0);                                                     \
    REQUIRE(_num_allocated_bytes() +                                           \
                aligned_size(_size_meta_data() * _num_allocated_blocks()) ==   \
...
template <typename T> void populate_array(T *array, size_t len) {
  for (size_t i = 0; i < len; i++) {
    array[i] = (T)i;
  }
}

template <typename T> void validate_array(T *array, size_t len) {
  for (size_t i = 0; i < len; i++) {
    REQUIRE((array[i] == (T)i));
  }
}
...
TEST_CASE("srealloc case a split", "[malloc3]") {
  ...
  sfree(b);
  verify_blocks(1, 32 + MIN_SPLIT_SIZE + _size_meta_data(), 1,
                32 + MIN_SPLIT_SIZE + _size_meta_data());
  verify_size(base);
}
...
TEST_CASE("srealloc case b", "[malloc3]") {
  ...
  sfree(new_b2);
  verify_blocks(1, 32 * 3 + 2 * _size_meta_data(), 1,
                32 * 3 + 2 * _size_meta_data());
  verify_size(base);
}
...
TEST_CASE("srealloc case b split", "[malloc3]") {
  ...
  REQUIRE(new_b != nullptr);
  REQUIRE(new_b == a);
  verify_blocks(2, MIN_SPLIT_SIZE + 32 * 3 + _size_meta_data(), 1,
                MIN_SPLIT_SIZE + 32 + _size_meta_data());
  verify_size(base);
  validate_array(new_b, 32);
...
TEST_CASE("srealloc case d split", "[malloc3]") {
  ...
  sfree(new_b);
  verify_blocks(2, MIN_SPLIT_SIZE + 32 * 3 + _size_meta_data(), 1,
                MIN_SPLIT_SIZE + 32 * 2 + _size_meta_data());
  verify_size(base);
...
TEST_CASE("srealloc case e", "[malloc3]") {
  ...
  sfree(new_b);
  verify_blocks(3, 32 * 3 + 2 * _size_meta_data() + pad_size, 1,
                32 * 3 + 2 * _size_meta_data());
  verify_size(base);

  sfree(pad1);
  sfree(pad2);
  verify_blocks(1, 32 * 3 + 4 * _size_meta_data() + pad_size, 1,
                32 * 3 + 4 * _size_meta_data() + pad_size);
  verify_size(base);
}
...
TEST_CASE("srealloc case e no pad", "[malloc3]") {
  ...
  sfree(new_b);
  verify_blocks(2, 32 * 3 + 2 * _size_meta_data() + pad_size, 1,
                32 * 3 + 2 * _size_meta_data());
  verify_size(base);

  sfree(pad1);
  verify_blocks(1, 32 * 3 + 3 * _size_meta_data() + pad_size, 1,
                32 * 3 + 3 * _size_meta_data() + pad_size);
  verify_size(base);
}
...
TEST_CASE("srealloc case e split", "[malloc3]") {
  ...
  REQUIRE(new_b != nullptr);
  REQUIRE(new_b == a);
  verify_blocks(4, blocks_size + _size_meta_data() + pad_size, 1,
                blocks_size - 512 + _size_meta_data());
  verify_size(base);
  validate_array(new_b, 104);

  sfree(new_b);
  verify_blocks(3, blocks_size + 2 * _size_meta_data() + pad_size, 1,
                blocks_size + 2 * _size_meta_data());
  verify_size(base);

  sfree(pad1);
  sfree(pad2);
  verify_blocks(1, blocks_size + 4 * _size_meta_data() + pad_size, 1,
                blocks_size + 4 * _size_meta_data() + pad_size);
  verify_size(base);
}
...
TEST_CASE("srealloc case fi", "[malloc3]") {
  ...
  sfree(new_b);
  verify_blocks(2, 32 * 4 + 2 * _size_meta_data() + pad_size, 1,
                32 * 4 + 2 * _size_meta_data());
  verify_size(base);

  sfree(pad1);
  verify_blocks(1, 32 * 4 + 3 * _size_meta_data() + pad_size, 1,
                32 * 4 + 3 * _size_meta_data() + pad_size);
  verify_size(base);
}
...
TEST_CASE("srealloc case fii", "[malloc3]") {
  ...
  sfree(new_b);
  verify_blocks(3, 32 * 4 + _size_meta_data() + pad_size, 1,
                32 * 3 + _size_meta_data());
  verify_size(base);
...
TEST_CASE("srealloc case g", "[malloc3]") {
  ...
  sfree(pad2);
  sfree(pad3);
  verify_blocks(1, blocks_size + pad_size + 4 * _size_meta_data(), 1,
                blocks_size + pad_size + 4 * _size_meta_data());
  verify_size(base);
}
...
TEST_CASE("srealloc case h", "[malloc3]") {
  ...
  sfree(pad2);
  sfree(pad3);
  verify_blocks(1, blocks_size + pad_size + 5 * _size_meta_data(), 1,
                blocks_size + pad_size + 5 * _size_meta_data());
  verify_size(base);
}
